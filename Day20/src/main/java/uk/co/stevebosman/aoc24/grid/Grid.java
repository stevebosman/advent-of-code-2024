...
public record Grid<T>(List<List<T>> grid, GridBounds bounds) {
  public Grid(final List<List<T>> grid) {
    this(grid, new GridBounds(Position.ORIGIN, new Position(grid.size() - 1, grid.getFirst().size() - 1)));
  }
  public Optional<T> get(final Position position) {
    if (bounds.contains(position)) {
      return Optional.of(grid.get(position.y()).get(position.x()));
    } else {
